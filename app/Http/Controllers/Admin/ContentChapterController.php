...
class ContentChapterController extends Controller
{
     ...
     * @return RedirectResponse
     */
    public function update(ChapterUpdateRequest $chapterUpdateRequest, Content $content): RedirectResponse
    {
        $this->chapterService->updateForContent($content, $chapterUpdateRequest->validated());
    ...
    {
        if ($content->chapter) {
            return inertia('Admin/Content/Title/Chapter/Show', [
                'content' => new ContentResource($content),
                'chapter' => new ChapterResource($content->chapter),
            ]);
        }

        return inertia('Admin/Content/Title/Chapter/Index', [
            'content' => new ContentResource($content),
        ]);
    }
}
