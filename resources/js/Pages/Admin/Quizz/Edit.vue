<script setup>
    ...
    const submit = () => {
        form.put(route("admin.quizz.update", form.id), {
            onSuccess: () => {
                $q.notify({
                    type: 'positive',
    ...
    function destroy() {
        $q.dialog({
            ...
            useForm().delete(route('admin.quizz.destroy', form.id), {
                onSuccess: () => {
                    $q.notify({
                        type: 'positive',
    ...
    const addAlternativeCreateQuestion = () => createQuestionForm.alternatives.push({
        name: null,
        is_correct: createQuestionForm.alternatives.length === 0
    })

    const removeAlternativeCreateQuestion = (position) => {
        let deletedCorrectAlternative = createQuestionForm.alternatives[position].is_correct;
        ...
        if (deletedCorrectAlternative) {
            createQuestionForm.alternatives[0].is_correct = true;
        }
    }

    const handleCorrectAlternativeCreateQuestion = (value, indexAlternative) => {
        createQuestionForm.alternatives.forEach((alt, indexLoop) => alt.is_correct = indexLoop === indexAlternative)
    }
    ...
    const storeQuestionSubmit = () => {
        ...
        createQuestionForm.alternatives.forEach(alt => newQuestion.alternatives.push({
            name: alt.name, is_correct: alt.is_correct
        }))
    ...
    const addAlternativeEditQuestion = () => editQuestionForm.alternatives.push({
        name: null,
        is_correct: editQuestionForm.alternatives.length === 0
    })

    const removeAlternativeEditQuestion = (position) => {
        let deletedCorrectAlternative = editQuestionForm.alternatives[position].is_correct;
        ...
        if (deletedCorrectAlternative) {
            editQuestionForm.alternatives[0].is_correct = true;
        }
    }

    const handleCorrectAlternativeEditQuestion = (value, indexAlternative) => {
        editQuestionForm.alternatives.forEach((alt, indexLoop) => alt.is_correct = indexLoop === indexAlternative)
    }
    ...
    const updateQuestionSubmit = () => {
        ...
        editQuestionForm.alternatives.forEach(alt => updateQuestion.alternatives.push({
            name: alt.name, is_correct: alt.is_correct
        }))
    ...
    <AuthenticatedLayout>
        ...
        <div class="row q-mb-lg">
            <div class="column col-12 col-md-6 justify-center">
                <div class="app-fs-28 app-fw-700 app-lh-32 text-grey-8"> Editar quizz </div>
                ...
                <q-card-section class="row q-col-gutter-lg">
                    ...
                    <div class="col-12" v-if="createQuestionForm.answer_type === 'fechada'">
                        <draggable
                            ...
                            item-key="index"
                        >
                            <template #item="{element: alternative, index}">
                                <div class="row q-col-gutter-lg q-mb-lg cursor-pointer">
                                        ...
                                        >
                                            ...
                                            <template v-slot:append>
                                                <q-checkbox
                                                    ...
                                                    color="green"
                                                    left-label
                                                    @update:model-value="handleCorrectAlternativeCreateQuestion($event, index)"
                                                />
                                            </template>
                ...
                <q-card-section class="row q-col-gutter-lg">
                    ...
                    <div class="col-12" v-if="editQuestionForm.answer_type === 'fechada'">
                        ...
                        >
                            <template #item="{element: alternative, index}">
                                <div class="row q-col-gutter-lg q-mb-lg cursor-pointer">
                                    <div class="col-md-7 col-11 offset-1">
                                        <q-input
                                            outlined
                                            :label="`Opção resposta ${numberToChar(index)}`"
                                            v-model="alternative.name"
                                        >
                                            ...
                                            <template v-slot:append>
                                                <q-checkbox
                                                    ...
                                                    color="green"
                                                    left-label
                                                    @update:model-value="handleCorrectAlternativeEditQuestion($event, index)"
                                                />
                                            </template>
