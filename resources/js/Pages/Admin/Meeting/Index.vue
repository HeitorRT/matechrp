<script setup>
    ...
    const props = defineProps({
        ...
        canMeetingIndividualView: Boolean
    });
    ...
    const removeStudent = (meeting, student) => {
        $q.dialog({
            component: DialogConfirm,
            componentProps: {
                title: 'Retirar aluno',
                message: `Tem certeza que deseja retirar ${student.name} do encontro ${meeting.name}?`,
            },
        }).onOk(() => {
            useForm().put(route('admin.meeting.detach-student', {
                ...
                onSuccess: () => {
                    $q.notify({
                        type: 'positive',
                        message: `${student.name} retirando do encontro ${meeting.name}.`,
                        position: 'top',
                    })
                }
            })
    ...
    const follow = (id) => useForm().get(route('admin.meeting.follow', id));

    const destroy = (meeting) => {
        $q.dialog({
            component: DialogConfirm,
            componentProps: {
                title: 'Excluir encontro',
                message: meeting.students.length > 0
                    ? `Há ${meeting.students.length} alunos agendados para o encontro, ao excluir, os agendamentos serão excluídos. Tem certeza que deseja excluir esse encontro?`
                    : "Tem certeza que deseja excluir esse encontro?",
            },
        }).onOk(() => {
            useForm().delete(route('admin.meeting.destroy', meeting.id), {
                onSuccess: () => {
                    $q.notify({
                        type: 'positive',
    ...
    const optionsStatus = computed(() => {
        ...
        for (const key in props.status) {
            options.push({ label: props.status[key], value: key })
        }
    ...
    const getColorStatus = (status) => {
        if (status === 'Aberto')   return 'text-grey';
        if (status === 'Iniciado') return 'text-green';
        return 'text-red';
    }
    ...
    <AuthenticatedLayout>
        ...
        <div class="row q-pb-xl">
            <div class="column col-12 col-md-6 justify-center">
                <div class="app-fs-28 app-fw-700 app-lh-32 text-grey-8"> Encontros </div>
        ...
        <q-card flat class="app-br-16">
            <q-card-section class="row items-center q-py-sm q-px-lg">
                ...
                <q-chip
                    class="adm-chip-primary"
                    v-if="query.filters.teacher_id && !canMeetingIndividualView"
                ...
                <q-chip
                    ...
                    :label="`Status = ${query.filters.status.map(s => status[s]).join(', ')}`"
                >
                    <q-icon
                        name="cancel"
                        size="xs"
                ...
                <q-btn round dense flat color="primary">
                    ...
                    >
                        <div class="row q-col-gutter-sm">
                            ...
                            <div class="col-12" v-if="!canMeetingIndividualView">
                                <q-select
            ...
            <q-card-section class="q-py-none">
                ...
                >
                    ...
                    <template v-slot:body-cell-status="props" >
                        <q-td :props="props">
                            <div
                                :class="getColorStatus(props.row.status)"
                                class="app-fw-500"
                            >
                                {{ props.row.status }}
                            </div>
                    ...
                    <template v-slot:body-cell-students="props">
                        <q-td :props="props">
                            <div v-for="student in props.row.students">
                                <q-chip
                                    class="adm-chip-primary"
                                    :label="student.name"
                                >
                                    <q-icon
                                        ...
                                        size="xs"
                                        class="q-ml-xs cursor-pointer"
                                        @click="removeStudent(props.row, student)"
                                        v-if="canMeetingDetachStudent && props.row.status != 'Finalizado'"
                                    />
                                </q-chip>
                            </div>
                        </q-td>
                    ...
                    <template v-slot:body-cell-actions="props">
                        <q-td :props="props">
                            <div class="row items-center justify-center app-fw-700 app-fs-16">
                                <q-btn icon="more_vert" flat v-if="(canMeetingEdit || canMeetingDestroy) && props.row.status !== 'Finalizado'">
                                    <q-menu :offset="[65, 0]">
                                        <q-list>
                                            <q-item
                                                v-if="canMeetingEdit"
                                            ...
                                            <q-item
                                                clickable
                                                @click="follow(props.row.id)"
                                                class="text-grey-7 flex flex-center"
                                            >
                                                <q-icon name="live_tv" size="xs"/>

                                                <q-item-section no-wrap>
                                                    <div class="q-ml-sm"> Acompanhar </div>
                                                </q-item-section>
                                            </q-item>
                                        </q-list>
                                    </q-menu>
