<script setup>
    ...
    const props = defineProps({
        ...
        canIndicationStore: Boolean,
        canIndicationEdit: Boolean,
    ...
    const submit = () => {
        requestData.get(route('admin.indication.index'), {
            preserveState: true,
            preserveScroll: true,
    ...
    const create = () => useForm().get(route('admin.indication.create'));

    const edit = (id) => useForm().get(route('admin.indication.edit', id));

    const show = (id) => useForm().get(route('admin.indication.show', id));
    ...
    function destroy(id) {
        $q.dialog({
            ...
            componentProps: {
                ...
                message: 'Tem certeza que deseja excluir essa indicação?',
            },
        }).onOk(() => {
            useForm().delete(route('admin.indication.destroy', id), {
                onSuccess: () => {
                    $q.notify({
                        type: 'positive',
    ...
    function destroySelected() {
        $q.dialog({
            ...
            componentProps: {
                ...
                message: 'Tem certeza que deseja excluir indicações selecionadas?',
            },
      }).onOk(() => {
        useForm({ ids: selected.value.map(s => s.id) }).post(route('admin.indication.destroy-multiples'), {
            onSuccess: () => {
                selected.value = [];

                $q.notify({
                    type: 'positive',
                    message: 'Indicações excluídas com sucesso',
                    position: 'top',
                })
            }
        })
    ...
    const clearFilters = () => useForm().get(route('admin.indication.index'))

</script>

<template>
    <AuthenticatedLayout>
        <Head title="Indicações" />

        <div class="row q-pb-xl">
            <div class="column col-12 col-md-6 justify-center">
                <div class="app-fs-28 app-fw-700 app-lh-32 text-grey-8"> Indicações </div>
                ...
                >
                    ...
                    <q-breadcrumbs-el label="Indicações" class="text-primary" />
                </q-breadcrumbs>
            </div>

             <div class="col-12 col-md-6 flex justify-end items-center">
                <q-btn
                    v-if="selected.length > 0 && canIndicationDestroy"
                    color="negative"
                    class="q-mr-md"
                    rounded
                    no-caps
            ...
            <q-card-section class="q-py-none">
                    ...
                    <template v-slot:body-cell-actions="props">
                        <q-td :props="props">
                            <div class="row items-center justify-center app-fw-700 app-fs-16">
                                <q-btn icon="more_vert" flat v-if="canIndicationEdit || canIndicationDestroy">
                                    <q-menu :offset="[65, 0]">
                                        <q-list>
                                            <q-item
                                                v-if="canIndicationEdit"
                                                clickable
                ...
                <div class="row items-center text-grey">
                    ...
                    <q-pagination
                        ...
                        :max="indications.meta.last_page"
                        @update:model-value="submit"
                        direction-links
                        boundary-links
                        color="grey"
